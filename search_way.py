...
from math import sqrt # lấy 1 hàm tính căn bậc 2
...
class Search():
    ...
    def reset(self):
        self.res=[]
        self.matrix_visit = np.array([False]*(self.w*self.h)).reshape(self.w, self.h)
    ...
    def bfs(self, start: tuple, goal: tuple):
        ...
        while len(queue) > 0:
            value = queue.pop(0) # lấy giá trị đầu tiên trong danh sách, đồng thời xóa đối tượng đó khỏi danh sách queue
            self.matrix_visit[value.x, value.y] = True # đánh dấu vị trí này vào ma trận kiểm tra
            ...
            for i in range(4):
                ...
                if self.checkMonsters(new_x, new_y):
                    tmp = Point(new_x, new_y) # tạo biến lưu trữ tạm thời điểm mới đó
                    ...
                    if self.checkVisited(new_x, new_y):
                        ...
                        queue.append(tmp) # thêm vị trí mới vào vào cuối danh sách queue
    ...
    def dfs(self, start: tuple, goal: tuple):
        ...
        stack = [S]
        while len(stack) > 0:
            value = stack.pop(-1) # lấy giá trị cuối cùng trong danh sách, đồng thời xóa đối tượng đó khỏi danh sách stack
            ...
            for i in range(4):
                ...
                if self.checkMonsters(new_x, new_y):
                    ...
                    if self.checkVisited(new_x, new_y):
                        tmp.par = value
                        stack.append(tmp) # thêm vị trí mới vào vào cuối danh sách stack
    ...
    def greedy(self, start, goal):
        self.reset()
        # khởi tạo điểm đầu với điểm cuối
        S = Point(start[0], start[1])
        G = Point(goal[0], goal[1])
        queue = [S] # tạo danh sách để chứa node
        dist = [S.distance(G)] # tạo danh sách chứa khoảng cách từ node đến đích
        while len(queue) != 0:
            idx = dist.index(min(dist)) # lấy vị trí tại giá trị min của dist
            value = queue.pop(idx) # lấy và xóa node
            dist.pop(idx) # xóa giá trị tương ứng với node
            self.matrix_visit[value.x, value.y] = True # cập nhật đường đã đi qua
            # kiểm tra vị trí đó có phải đích chưa
            if value.x == G.x and value.y == G.y:
                return self.path(value)
